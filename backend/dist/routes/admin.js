...
router.post('/login', async (req, res) => {
    ...
    if (username === adminUser && password === adminPass) {
        const token = jsonwebtoken_1.default.sign({ role: 'admin', username }, getJwtSecret(), { expiresIn: '7d' });
...
router.get('/tenants', adminAuth, async (req, res) => {
    try {
        const tenants = await models_1.Tenant.find().sort({ createdAt: -1 });
        const tenantList = [];
        for (const tenant of tenants) {
            const t = tenant;
            // Calculate student count
            const studentCount = await models_1.Student.countDocuments({ tenantId: t._id });
 ...
 */
router.post('/generate-license', adminAuth, async (req, res) => {
    ...
    try {
        ...
        const key = (0, license_1.generateLicenseKey)(tenantId, expiresAt, type);
...
router.post('/revoke-license', adminAuth, async (req, res) => {
